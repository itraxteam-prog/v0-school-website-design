            ...
            <div className="flex flex-col gap-8 pb-8">
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {initialClasses.length === 0 ? (
                        <div className="col-span-full text-center py-10 text-muted-foreground">No classes assigned.</div>
                    ) : (
                        initialClasses.map((cls) => (
                            <Card key={cls.id} className={cn("glass-card group overflow-hidden border-border/50 hover:border-primary/30", "bg-gradient-to-br", cls.color)}>
                                ...
                                <CardContent className="space-y-5">
                                    <div className="flex flex-col gap-3">
                                        <div className="flex items-center gap-2 text-sm text-foreground/80"><Users size={16} className="text-primary" /><span>{cls.studentCount} Students Enrolled</span></div>
                                        <div className="flex items-center gap-2 text-sm text-foreground/80"><MapPin size={16} className="text-primary" /><span>{cls.room}</span></div>
                                    </div>
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-end">
                                            <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider"><TrendingUp size={14} className="text-emerald-500" />Average Performance</div>
                                            <span className="text-sm font-bold text-foreground">{cls.performance}%</span>
                                        </div>
                                        <div className="h-1.5 w-full bg-muted/50 rounded-full overflow-hidden">
                                            <div className="h-full bg-primary transition-all duration-1000 ease-out" style={{ width: `${cls.performance}%` }} />
                                        </div>
                                    </div>
                                </CardContent>
                                <CardFooter className="pt-2 border-t border-border/30 bg-muted/10">
                                    <Button variant="ghost" className="w-full justify-between items-center group-hover:bg-primary group-hover:text-white transition-all text-sm font-semibold" onClick={() => window.location.href = '/portal/teacher/attendance'}>
                ...
                <div className="mt-4 rounded-xl border border-primary/10 bg-primary/5 p-4 md:p-6 lg:flex lg:items-center lg:justify-between gap-6">
                    <div className="flex items-center gap-4">
                        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-primary text-white shadow-lg"><CalendarCheck size={24} /></div>
                        <div>
                            <h4 className="font-semibold text-foreground">Weekly Overview</h4>
                            <p className="text-sm text-muted-foreground mt-0.5">You have {initialClasses.length} active classes assigned.</p>
                        </div>
                    </div>
