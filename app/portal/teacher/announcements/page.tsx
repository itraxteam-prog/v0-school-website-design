...
export default function TeacherAnnouncementsPage() {
    ...
    const fetchAnnouncements = useCallback(async () => {
        ...
        setError(null)
        try {
            const annRes = await fetch("/api/teacher/announcements")
            ...
            // Optionally fetch stats/events if teacher has them
            const statsRes = await fetch("/api/teacher/stats")
            if (statsRes.ok) {
                const statsData = await statsRes.json()
                // Use whatever upcoming events logic exists for teachers
                setEvents(statsData.upcomingEvents || [])
            }
            ...
            <AnimatedWrapper direction="down">
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Stay informed with the latest updates from administration.</p>
                    </div>
                                                    ...
                                                    <div className="flex justify-between items-start gap-4">
                                                        <div className="flex flex-col gap-1.5 flex-1">
                                                            ...
                                                            <div className="flex items-center gap-3 text-xs text-muted-foreground">
                                                                ...
                                                                <span className="flex items-center gap-1">• From: {a.author}</span>
                                                            </div>
                                                        </div>
                        ...
                        <CardContent className="p-0">
                            <div className="flex flex-col">
                                {loading ? (
                                    ...
                                    <div className="p-8 text-center text-xs text-muted-foreground italic text-opacity-50">
